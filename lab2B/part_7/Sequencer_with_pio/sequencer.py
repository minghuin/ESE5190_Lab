...
def recordFromConsole(recordLength, sp, destRegister, destMask):
    ...
    while len(consoleContent) < recordLength:
        if(sp.in_waiting > 0):
            ...
            if serialStringDecoded != "\r\n":
                ...
                elif ("Mask" in serialStringDecoded) and ('Value' in serialStringDecoded):
            ...
            if len(currentRecordDict.keys()) == 7 and currentRecordDict not in consoleContent:
                if currentRecordDict['MaskValue'] == destMask and currentRecordDict['Address'] == destRegister:
...
def recordFromConsole_pio(recordLength, sp):
    # set the REPL to read mode
    sp.write(b'R')
    consoleContent = []
    currentRecordDict = {}
    ...
    while len(consoleContent) < recordLength:
        if(sp.in_waiting > 0):
            ...
            if serialStringDecoded != "\r\n" and "pin" in serialStringDecoded:
                print(serialStringDecoded)
                #print(commendElementLst)
                #['pin', 'num:', '23|', 'data', 'bit:', '0|', 'time:4015184', '\r\n']
                if "22" in serialStringDecoded:
                    # create a new record
                    currentRecordDict["pin number"] = 22
                else:
                    currentRecordDict["pin number"] = 23
                currentRecordDict["data bit"] = commendElementLst[5][0:-1]
                currentRecordDict["time"] = commendElementLst[6][5:]

            print(currentRecordDict)

            if len(currentRecordDict.keys()) == 3 and currentRecordDict not in consoleContent:
                consoleContent.append(currentRecordDict)
            else:
                currentRecordDict = {}
                #    consoleContent.append(serialString)
...
def writeToConsole(factor, loopNum, writeData, sp):
    # setup the speed
    if factor >= 0:
...
addressLst = list('d0000004')
maskLst = list('00200000')
currentRecordDict = {}
...
if(mode == 'record'):
    ...
    # TODO get the input of address, mask
    ...
    while(1):
        dataType= input("Plase input content you want to record(pio/register):")
        if(dataType == 'pio' or dataType == 'register'):
            break
        else:
            pass
    
    if(dataType == 'register'):
        print("start recording register value after 5 seconds")
        time.sleep(5)
        print("recording...")
        records = recordFromConsole(recordLength = recordLen, sp = serialPort, destRegister= ''.join(addressLst), destMask= ''.join(maskLst))
        print("Record Completed")
        b = json.dumps(records)
        f2 = open('/home/ncm656112/ESE519_Lab/lab2A/Sequencer/recorded_register_data.json','w')
        f2.write(b)
        f2.close()
        print("Recording finished, please go to /home/ncm656112/ESE519_Lab/lab2A/Sequencer/recorded_data.json to check it")
    else:
        print("start recording pio value after 5 seconds")
        time.sleep(5)
        print("recording...")
        # TODO reading function
        pio_records = recordFromConsole_pio(recordLength = recordLen, sp = serialPort)
        print("pio-recording compeleted")
        b = json.dumps(pio_records)
        f2 = open('/home/ncm656112/ESE519_Lab/lab2A/Sequencer/recorded_pio_data.json','w')
        f2.write(b)
        f2.close()
        print("Recording finished, please go to /home/ncm656112/ESE519_Lab/lab2A/Sequencer/recorded_pio_data.json to check it")
        # TODO save data in the file 

elif(mode == 'replay'):
    while(1):
        factor = input("Plase input the replay speed factor n, the final speed will be timed 2^n :")
        if(factor.isdigit()):
            factor = int(factor)
            break
        else:
    ...
    writeToConsole(factor, count, data, serialPort)
